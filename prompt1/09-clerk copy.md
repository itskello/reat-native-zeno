...
Study the existing ZENO auth screens and current mocked auth flow (Where Do 
You Live → Create Account → Verify Phone Number → Verify Email → Set Up PIN 
→ Biometric/Fingerprint Setup → Onboarding Complete, plus Log In and Forgot 
Password), then replace all mock behavior with real Clerk authentication by 
following the Clerk Expo documentation provided below.

Keep the existing UI and navigation flow fully intact — do not change any 
screen design, layout, spacing, colors, or copy. If a design or flow decision 
needs to change to make Clerk work correctly, ask me before implementing 
anything — do not guess or silently deviate.

Implement, using Clerk:
- Create Account: email + phone + password sign-up.
- Login With Google: Google OAuth social sign-in.
- Log In: email/phone + password sign-in.
- Verify Phone Number: phone OTP verification (auto-sent after Create 
  Account submission).
- Verify Email: email OTP verification (auto-sent after phone verification 
  succeeds) — skipped entirely for Google sign-ups, since Google already 
  verifies email ownership.
- Forgot Password / Verify Reset Code / Reset Password: code-based password 
  reset flow (not link-based).

Session and routing logic:
- If the user is authenticated (valid Clerk session) → show the home route (/).
- If the user is not authenticated → show the onboarding/auth route 
  (/onboarding or /(auth)/welcome, matching whatever the current routing 
  convention is in this project).
- After successful full verification (phone + email for direct sign-up, or 
  phone only for Google sign-up) → proceed to Set Up PIN, then Biometric 
  Setup, then Onboarding Complete, then navigate to the home route (/).
- Do not navigate to the home route until both required verifications are 
  complete — an authenticated-but-unverified Clerk session should still be 
  routed through the remaining verification steps, not straight to home.

Security note: PIN and biometric setup are ZENO-specific security layers on 
top of Clerk's session — they are not part of Clerk's own auth flow. Store 
PIN verification status appropriately per AGENTS.md fintech security rules 
(never validate the PIN in plain text on the client without backend 
confirmation) — flag if Clerk's infrastructure doesn't cleanly support this 
and a custom backend endpoint is needed instead.

Do not change any screen design. If there is any ambiguity or a design/flow 
tradeoff is required to implement this correctly, ask me before implementation.

(Here paste the latest Clerk Expo documentation: 
